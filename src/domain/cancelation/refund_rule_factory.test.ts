...
describe("RefundRuleFactory", () => {
  ...
  it("deve retornar PartialRefund quando a reserva for cancelada entre 1 e 7 dias de antecedência", () => {
    const rule1 = RefundRuleFactory.getRefundRule(7);
    expect(rule1).toBeInstanceOf(PartialRefund);

    const rule2 = RefundRuleFactory.getRefundRule(1);
    expect(rule2).toBeInstanceOf(PartialRefund);
  });
